module.exports.config = {
  ...
  description: "Filter deactivated/deleted/disabled/facebook user accounts.",
  hasPrefix: true
};
...
module.exports.run = async function({ api, event }) {
  ...
  if (filteredUsers.length === 0) {
    api.sendMessage("Your group does not exist 'Facebook User'.", event.threadID, event.messageID);
  } else {
      ...
      if (isBotAdmin) {
        api.sendMessage("Starting filtering...\n\n", event.threadID, async () => {
          ...
          api.sendMessage(`✅ Successfully filtered ${successCount} people.`, event.threadID, () => {
            if (failCount !== 0) {
              api.sendMessage(`❌ Failed to filter ${failCount} people.`, event.threadID, event.messageID);
            }
          }, event.messageID);
        });
      } else {
        api.sendMessage("Bot is not an admin, so it can't filter.", event.threadID, event.messageID);
      }
    }, event.messageID);
  }
};